    ...
    if args.cv_inf:
        ...
        for agg_method, K in hparam_configs:
            ...
            metric2val, agg2metrics = run_inference_model(
                args, inf_model_name, base_models, 'dev', args.opt_normspace,
                agg_method, K
            )
        ...
        metric2val, agg2metrics = run_inference_model(
            args, inf_model_name, base_models, 'test', args.opt_normspace,
...
with open(os.path.join(args.output_dir, f'results_{args.dataset_name}.csv'), 'w') as fp:

    fp.write('\nModel Name, MAE, CRPS, MSE, DTW, TDI')
...
with open(os.path.join(args.output_dir, f'results_agg_{args.dataset_name}.csv'), 'w') as fp:

    fp.write('\nModel Name, AGG, K, MAE, CRPS, MSE')
    for model_name, agg2metrics in model2aggmetrics.items():
        for agg, K2metrics in agg2metrics.items():
            for K, metrics_dict in K2metrics.items():
                fp.write(
                    f"\n{model_name}, {agg}, {K}, {metrics_dict['mae']:.6f}, {metrics_dict['crps']:.6f}, {metrics_dict['mse']:.6f}"
                )

for model_name, metrics_dict in model2metrics.items():
    for metric, metric_val in metrics_dict.items():
        model2metrics[model_name][metric] = str(metric_val)
with open(os.path.join(args.output_dir, f'results_{args.dataset_name}.json'), 'w') as fp:
...
with open(os.path.join(args.output_dir, f'results_base_{args.dataset_name}.csv'), 'w') as fp:

    fp.write('\nAGG, K, model name, MAE, CRPS, MSE, DTW, TDI')
...
for model_name, metrics_dict in model2metrics.items():
    for metric, metric_val in metrics_dict.items():
        model2metrics[model_name][metric] = str(metric_val)
with open(os.path.join(args.output_dir, f"results_base_{args.dataset_name}.json"), 'w') as fp:
    json.dump(model2metrics, fp)
